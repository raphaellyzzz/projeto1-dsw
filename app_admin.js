...
createApp({
  ...
  computed: {
    ...
    encomendasFiltradasC() {
      ...
      const filtroPesoMin = parseFloat(this.filtroPesoMinEncomenda) || 0;
      const filtroPesoMax = parseFloat(this.filtroPesoMaxEncomenda) || Infinity;

      return this.listaEncomendas.filter(e => {
        const condCodigo = (e.codigo || '').toLowerCase().includes(filtroCodigo);
        const condStatus = (e.status || '').toLowerCase().includes(filtroStatus);
        const condTipo = (e.tipo || '').toLowerCase().includes(filtroTipo);
        const condPeso = (parseFloat(e.peso) >= filtroPesoMin && parseFloat(e.peso) <= filtroPesoMax);

        return condCodigo && condStatus && condTipo && condPeso;
      });
    }
  },
  methods: {
    ...
    obterNomeCliente(id) {
      const cliente = this.listaClientes.find(c =>
        String(c.id) === String(id) || String(c.cpfCnpj) === String(id)
      );
      return cliente ? cliente.nome : (id ? `Cliente ${id}` : 'N/A');
    },
    ...
    async adicionarCliente(event) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          console.error('Erro ao adicionar cliente:', res.status, res.statusText, errorText);
        }
      } catch (e) {
        console.error('Erro na requisição (catch):', e);
      }
    },
    ...
    async adicionarRota() {
      const payload = {
        ...
        centros: this.novaRotaForm.centros ? this.novaRotaForm.centros.split(',').map(c => c.trim()) : [],
